    ...
    struct promise_type: promise_base_type<promise_type> {
        struct final_awaiter {
            ...
            coroutine_handle<> await_suspend(coroutine_handle<promise_type> h) noexcept {
                return h.promise().continuation ? h.promise().continuation : noop_coroutine();
            }
...
///////////////////////////////////////////////////////////////////////////////
    struct awaiter {
        ...
        await_suspend(coroutine_handle<promise_type> continuation) {
...
task loop_synchronously(size_t count) {
    ...
    for (size_t i = 0; i < count; ++i)
        co_await completes_synchronously(); // fix stack overflow!
